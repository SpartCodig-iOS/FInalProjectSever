...
- 기존 `swagger-output.json` 을 그대로 사용합니다.
- 로컬/도커/배포 환경 모두 `/api-docs` 에 동일한 문서가 노출됩니다.
- 스펙을 갱신하려면 `swagger.js` 등을 이용해 `swagger-output.json` 을 새로 생성하고 커밋하면 됩니다.
...
- **Swagger 스펙은 어디서 수정하나요?**  
  루트의 `swagger-output.json` 을 갱신하고 서버/도커를 다시 시작하면 됩니다.
